...
def remitos_entregas():
    ...
    if cliente_selection:
        ...
        if not matching_client.empty:
            ...
            # st.session_state.cabecera_data['porc_dto'] = client_data['porc_dto']
        ...
        # Solo limpiar si realmente se cambió de tener selección a no tener
        st.session_state.cabecera_data['cliente_id'] = None
        # st.session_state.cabecera_data['porc_dto'] = None
        st.session_state.cliente_selected_display = None
        # st.session_state.porc_dto = None
    ...
    if st.session_state.cabecera_data.get('cliente_id') and st.session_state.cliente_selected_display:
        # Extraer solo la razón social del display name
    ...
    if not st.session_state.items_data.empty:
        st.dataframe(
            st.session_state.items_data[['Articulo', 'Descripción', 'Entregados', 'Observaciones']],
            hide_index=True,
            column_config={
    ...
    if is_remito_saved and not st.session_state.get('success_shown', False):
